...
open class BucketService(private val bucketStore: BucketStore, private val objectStore: ObjectStore) : ServiceBase() {
    ...
    fun listBuckets(
        ...
        var buckets = bucketStore
            .listBuckets()
            .stream()
            .filter { b: BucketMetadata? -> b!!.name.startsWith(normalizedPrefix) }
            .sorted(Comparator.comparing(Function { b: BucketMetadata -> b.name }))
            .map { bucketMetadata: BucketMetadata? -> Bucket.from(bucketMetadata) }
            .toList()

        if (bucketRegion != null) {
            buckets = buckets
                .stream()
                .filter { b: Bucket? -> b!!.bucketRegion == bucketRegion.toString() }
                .toList()
        }

        if (continuationToken != null) {
            val continueAfter = listBucketsPagingStateCache[continuationToken]
            buckets = filterBy<Bucket>(buckets, Bucket::name, continueAfter)
    ...
    fun getS3Objects(bucketName: String, prefix: String?): List<S3Object> {
        val bucketMetadata = bucketStore.getBucketMetadata(bucketName)
        val uuids: List<UUID?> = bucketStore.lookupIdsInBucket(prefix, bucketName)
        return uuids
            .stream()
            .map<S3ObjectMetadata?> { uuid: UUID? -> objectStore.getS3ObjectMetadata(bucketMetadata, uuid!!, null) }
            .filter { obj: S3ObjectMetadata? -> Objects.nonNull(obj) }
            .map { s3ObjectMetadata: S3ObjectMetadata? -> S3Object.from(s3ObjectMetadata) }  // List Objects results are expected to be sorted by key
    ...
    fun listVersions(
        ...
        return ListVersionsResult(
            result.commonPrefixes, deleteMarkers, delimiter, result.encodingType,
    ...
    fun listObjectsV2(
        ...
        if (maxKeys == 0) {
            return ListBucketResultV2(
                mutableListOf<Prefix>(), mutableListOf<S3Object>(), continuationToken,
        ...
        if ("url" == encodingType) {
            contents = mapContents(
              ...
              S3Object(
                ...
                it.lastModified,
                it.owner, it.restoreStatus, it.size,
                it.storageClass
              )
            }
    ...
    fun listObjectsV1(
        ...
        if (maxKeys == 0) {
            return ListBucketResult(
                mutableListOf<Prefix>(), mutableListOf<S3Object>(), null, encodingType,
