...
open class BucketStore(
    ...
    fun listBuckets(): MutableList<BucketMetadata> {
        return findBucketPaths()
            .stream()
            .map { path: Path? -> path!!.fileName.toString() }
            .map { bucketName: String? -> this.getBucketMetadata(bucketName!!) }
            .toList()
    }
    ...
    private fun <R> lookupInBucket(
        prefix: String?,
        bucketName: String,
        extract: Function<MutableMap.MutableEntry<String, UUID>, R>
    ): List<R> {
        val bucketMetadata = getBucketMetadata(bucketName)
        ...
        synchronized(lockStore[bucketName]!!) {
            return bucketMetadata.objects
                .entries
                .stream()
                .filter { entry: Map.Entry<String, UUID>? -> entry!!.key.startsWith(normalizedPrefix) }
                .map<R>(extract)
                .toList()
        }
    }
    ...
    private fun findBucketPaths(): List<Path> {
        ...
        try {
            Files.newDirectoryStream(
                rootFolder.toPath(),
                DirectoryStream.Filter { path: Path? -> Files.isDirectory(path) }).use { stream ->
                for (path in stream) {
        ...
        for (bucketName in bucketNames) {
            ...
            synchronized(lockStore[bucketName]!!) {
                ...
                for (objectEntry in objects.entries) {
                    objectIds.add(objectEntry.value)
                    LOG.info("Loading existing bucket {} key {}", bucketName, objectEntry.key)
                }
            }
        }
        return objectIds
